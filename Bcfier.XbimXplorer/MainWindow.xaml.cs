...
namespace Bcfier.XbimXplorer
{
  ...
  public partial class MainWindow : IXbimXplorerPluginWindow //, IXbimXplorerPluginMessageReceiver
  {
    ...
    private void OnOpenView(object sender, ExecutedRoutedEventArgs e)
    {
      ...
      if (view.PerspectiveCamera != null)
      {
        // todo: this works internally, but we must ensure it's compatible with other bcf viewers
        var pc = view.PerspectiveCamera;
        position = new XbimPoint3D(pc.ViewPointX, pc.ViewPointY, pc.ViewPointZ);
        direction = new XbimPoint3D(pc.DirectionX, pc.DirectionY, pc.DirectionZ);
        upDirection = new XbimPoint3D(pc.UpX, pc.UpY, pc.UpZ);
      ...
      else if (view.OrthogonalCamera != null)
      {
        // todo: this works internally, but we must ensure it's compatible with other bcf viewers
        var pc = view.OrthogonalCamera;
      ...
      if (view.ClippingPlanes != null && view.ClippingPlanes.Any())
      {
        var curP = view.ClippingPlanes[0];
    ...
    private void OnAddView(object sender, ExecutedRoutedEventArgs e)
    {
      try
      {
        ...
        var dialog = new AddViewXbim(issue, Path.Combine(Path.GetTempPath(), Bcfier.SelectedBcf().Id.ToString()), _xpWindow.DrawingControl)
        {
        ...
        var generatedViewpoint = GenerateViewpoint(_xpWindow.DrawingControl);
    ...
    private static BcfViewpointViewModel GenerateViewpoint(DrawingControl3D control3D)
    {
      try
      {
        ...
        if (control3D.Viewport.Orthographic)
        {
          ...
          if (cam != null)
            ...
            {
              // todo: this works internally, but we must ensure it's compatible with other bcf viewers
              ViewPointX = cam.Position.X,
              ViewPointY = cam.Position.Y,
              ViewPointZ = cam.Position.Z,
              UpX = cam.UpDirection.X,
              UpY = cam.UpDirection.Y,
              UpZ = cam.UpDirection.Z,
              DirectionX = cam.LookDirection.X,
              DirectionY = cam.LookDirection.Y,
              DirectionZ = cam.LookDirection.Z,
